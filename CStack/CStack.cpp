...
//------------------<< Defining macros >>------------------------
#define ERROR_DETECT(condition, error_name, return);\
if (!condition)                                     \
{                                                   \
    stack_errno = error_name;                       \
    dump();                                         \
    assert((#condition, 0));                        \
    return;                                         \
}                                                   \
...
{
    ERROR_DETECT((this != nullptr),        ERR_NULL, return 0);
    ERROR_DETECT((data_ != nullptr),       ERR_DATA, return 0);
    ERROR_DETECT((top_ <= size_),          ERR_OVFL, return 0);
    ERROR_DETECT((top_ >= 0),              ERR_TOP_, return 0);
    ERROR_DETECT((size_ < MAX_STACK_SIZE), ERR_MAX_, return 0);
